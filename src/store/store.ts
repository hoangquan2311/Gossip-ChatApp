...
const useStore = defineStore("chat", () => {
  ...
  // auth refs
  const accessToken: Ref<string | null> = ref(storage.accessToken || null);
  ...
  const clearTokens = () => {
    ...
    delete updatedStorage.accessToken;
    localStorage.setItem("chat", JSON.stringify(updatedStorage));
  };
